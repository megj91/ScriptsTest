...
public class StoreSubscriptions : MonoBehaviour
{
    int shardsQuantity;
    int coinsQuantity;
    ...
    void LoadData()
    {
        ...
        foreach (var item in ServiceLocator.Instance.GetService<ICurrencyShop>().GetData().GetCurrencyDataList())
        {
            ...
            GameObject optionObject = Instantiate((GameObject)Resources.Load(ViewsDictionary.ComponentsDictionary[ComponentsDirectoryEnum.UI_STORE_SUBSCRIPTIONS]), optionScroll.content);
            optionsList.Add(optionObject);

            optionObject.transform.Find("Quantity").GetComponent<TextMeshProUGUI>().text = item._currencyValue.ToString() + " COINS";
