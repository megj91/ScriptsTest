...
public class StoreShards : MonoBehaviour
{
    int shardsQuantity;
    int coinsQuantity;
    ...
    void LoadData()
    {
        ...
        foreach (var item in ServiceLocator.Instance.GetService<ICurrencyShop>().GetData().GetCurrencyDataList())
        {
            ...
            GameObject optionObject = Instantiate((GameObject)Resources.Load(ViewsDictionary.ComponentsDictionary[ComponentsDirectoryEnum.UI_STORE_SHARD]), optionScroll.content);
